...
The project **connector-rabbitmq** is a Bonita Connector for **Bonita {short-bonita-version}**  **Community** version written in `java` language.
...
[source,bash]
----
git clone https://your.github.com/connector-rabbitmq.git
cd connector-rabbitmq/
./mwnw package
----

The build should produce a jar archive under the `target/` folder named `connector-rabbitmq-1.0-SNAPSHOT.jar`
